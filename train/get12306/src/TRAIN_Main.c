...
int MAIN_Parse_Train_JSON(char *pcData)
{
    ...
    for (uint32_t i = 0; i < u32ResultSize; i++)
    {
        ...
        if (cjTemp != NULL && cjTemp->type == cJSON_String)
        {
            ...
            bzero(aStart, sizeof(aStart));
            bzero(aEnd, sizeof(aEnd));
            bzero(aFrom, sizeof(aFrom));
            bzero(aTo, sizeof(aTo));
            bzero(aStartTime, sizeof(aStartTime));
            bzero(aEndTime, sizeof(aEndTime));
            bzero(aAllTime, sizeof(aAllTime));

            bzero(aYiDeng, sizeof(aYiDeng));
            bzero(aErDeng, sizeof(aErDeng));
            bzero(aTeDeng, sizeof(aTeDeng));
            bzero(aDongWo, sizeof(aDongWo));

            bzero(aGaoRuanWo, sizeof(aGaoRuanWo));
            bzero(aYiRuanWo, sizeof(aYiRuanWo));
            bzero(aYingWo, sizeof(aYingWo));
            bzero(aRuanZuo, sizeof(aRuanZuo));
            bzero(aYingZuo, sizeof(aYingZuo));
            bzero(aWuZuo, sizeof(aWuZuo));
            ...
            for (uint8_t j = 0; j < 48; j++)
            {
                ...
                while (u32Index < u32Len && aTempBuff[u32Index] != '|')
                    u32Index++;
                u32Index++;
            }
